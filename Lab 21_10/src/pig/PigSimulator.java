...
public class PigSimulator {
	...
	public static void main (String [ ] args) {
		...
		players[2] = new PlayerRunner ( );
		players[3] = new Player2Runner ( );
	...
	public int play1game (Player [ ] players) {
		...
		players[2].initGame ( );
		players[3].initGame ( );
		int currentPlayer = r.nextInt (4);//(currentPlayer+1)%4
		boolean wonGame = false;
		while (!wonGame) {
			currentPlayer = (currentPlayer+1)%4;
			System.out.println ("player " + currentPlayer + ": score = " + players[currentPlayer].total ( ));
			play1turn (players[currentPlayer], players[(currentPlayer+1)%4]);
